...
static double cmd_rem(const double *args) {
...
static double cmd_hypotenuse(const double *args) {
...
static double cmd_eq(const double *args) {
    return args[0] == args[1];
}

static double cmd_neq(const double *args) {
    return args[0] != args[2];
}

static double cmd_lt(const double *args) {
    return args[0] < args[1];
}

static double cmd_gt(const double *args) {
    return args[0] > args[1];
}

static double cmd_lte(const double *args) {
    return args[0] <= args[1];
}

static double cmd_gte(const double *args) {
    return args[0] >= args[1];
}

static const MenuCommand cmds_relational[] = {
    {"eq", cmd_eq, 2},
    {"neq", cmd_neq, 2},
    {"lt", cmd_lt, 2},
    {"gt", cmd_gt, 2},
...
static double cmd_parallel(const double *args) {
    return 1.0 / ((1.0 / args[0]) + (1.0 / args[1]));
}

static double cmd_impedance(const double *args) {
    return sqrt((args[0] * args[0]) + (args[1] * args[1]));
}

static const MenuCommand cmds_electronics[] = {
    {"parallel", cmd_parallel, 2},
    {"impedance", cmd_impedance, 2},
    {0},
};
...
static const MenuFolder folders_root[] = {
    ...
    {"relational", folders_none, cmds_relational},
    {"bitwise", folders_none, cmds_bitwise},
    {"electronics", folders_none, cmds_electronics},
    {0},
};
...
static bool menu_init_folder(const MenuFolder *folder) {
    for (const MenuCommand *command = folder->commands; command->name != NULL; command++) {
        if (!hm_put(menu_command_map, command->name, command)) {
            return false;
        }
    }
